...
func TestStoreListAccountSummariesWithoutCursor(test *testing.T) {
	test.Parallel()
	...
	stubPool := &stubPool{
		queryRowsFn: func(ctx context.Context, sql string, arguments ...any) (queryRows, error) {
			...
			return &stubRows{
				records: [][]any{
					{uuid.NewString(), "user-1"},
					{uuid.NewString(), "user-2"},
				},
			}, nil
		},
	}
	...
	accounts, err := store.ListAccountSummaries(context.Background(), tenantID, ledgerID, nil, 2)
	if err != nil {
		test.Fatalf("list accounts: %v", err)
	}
	if capturedSQL != sqlListAccountSummaries {
		test.Fatalf("expected sqlListAccountSummaries, got %q", capturedSQL)
	}
	if len(capturedArgs) != 3 {
		test.Fatalf("expected 3 args, got %d", len(capturedArgs))
	}
	if capturedArgs[0] != tenantID.String() || capturedArgs[1] != ledgerID.String() || capturedArgs[2] != 2 {
		test.Fatalf("unexpected args: %+v", capturedArgs)
	}
	if len(accounts) != 2 {
		test.Fatalf("expected 2 accounts, got %d", len(accounts))
	}
	if accounts[0].TenantID() != tenantID || accounts[0].LedgerID() != ledgerID {
		test.Fatalf("unexpected account: %+v", accounts[0])
	}
}

func TestStoreListAccountSummariesWithCursor(test *testing.T) {
	test.Parallel()
	tenantID := mustTenantID(test)
	ledgerID := mustLedgerID(test)
	afterAccountID, err := ledger.NewAccountID(uuid.NewString())
	if err != nil {
		test.Fatalf("after account id: %v", err)
	}
	...
	accounts, err := store.ListAccountSummaries(context.Background(), tenantID, ledgerID, &afterAccountID, 50)
	if err != nil {
		test.Fatalf("list accounts: %v", err)
	}
	if len(accounts) != 0 {
		test.Fatalf("expected no accounts, got %d", len(accounts))
	}
	if capturedSQL != sqlListAccountSummariesAfter {
		test.Fatalf("expected sqlListAccountSummariesAfter, got %q", capturedSQL)
	}
	if len(capturedArgs) != 4 {
		test.Fatalf("expected 4 args, got %d", len(capturedArgs))
	}
	if capturedArgs[0] != tenantID.String() || capturedArgs[1] != ledgerID.String() || capturedArgs[2] != afterAccountID.String() || capturedArgs[3] != 50 {
		test.Fatalf("unexpected args: %+v", capturedArgs)
	}
}
