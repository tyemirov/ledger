...
- [x] [LG-406] Establish github workflows for testing and docker image release. Use an example under @docs/workflow for inspiration. use make commands instead of calling go or other tools directly
    - 2025-11-19: Added `.github/workflows/ci.yml` to enforce `go mod tidy` drift checks plus `make ci` on pushes and pull requests, and introduced `.github/workflows/docker-image.yml` to build and publish the Dockerfile output to `ghcr.io/${{ github.repository_owner }}/ledger` on master pushes or manual runs.
