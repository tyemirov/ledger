...
- [x] [B307] (P0) Make the canonical release, publish, and deploy lifecycle safely idempotent.
  ...
  Resolved 2026-07-28: exact-tag release preparation now verifies the one-parent CHANGELOG-only release and exits before CI/artifact mutation; GitHub metadata/assets and versioned platform images are immutable, with publication limited to missing state; completed releases with absent local staging are verified from the tagged commit, GitHub manifest/assets, exact GHCR platform/version manifests, and `latest`; deploy image lookup failures are explicit before the convergent gateway transaction. Every release entrypoint explicitly invokes the dependency-free helper through Python 3, removing the undeclared `uv` runtime dependency exposed by the hosted Ubuntu check. Black-box retry/recovery and runtime-portability coverage, `make test`, `make lint`, and `make ci` pass with 100% production coverage. A disposable clean clone also verified the real published `v1.0.3` GitHub and GHCR state without mutation; no publish or deployment command ran.
