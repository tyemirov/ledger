...
- Release and deployment artifacts originate from the repository-owned prepared-artifact pipeline; GitHub Actions validates source but does not own production publication or deployment.
...
CI validation is defined by workflows in `.github/workflows/`; release preparation, publication, and deployment remain explicit repository-owned local stages.
