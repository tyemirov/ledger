...
- `demo/frontend/ui` contains the static bundle that references `mpr-ui` via CDN. While LG-301 is open it renders a login-only experience to debug TAuth; ledger calls remain available through the backend but are no longer triggered from the page.
...
- `demo/.env.tauth.example` exports the Google OAuth Web Client ID consumed by `demo/frontend/ui/index.html` via `/demo/config.js`. Copy the example files, fill in secrets, and restart the stack whenever you rotate credentials. When registering that OAuth Web client in Google Cloud Console, add **both** `http://localhost:8000` (the Nginx proxy that serves the UI + `/auth/*`) and `http://localhost:8080` (direct access to TAuth) to the “Authorized JavaScript origins” list so Google Identity Services accepts the nonce flow described in `docs/TAuth/usage.md`.
